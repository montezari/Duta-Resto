...
if($handle){
  ...
  $font = printer_create_font("Times New Roman", 15, 12, PRINTER_FW_NORMAL, false, false, false, 0);
  //$font = printer_create_font("Arial", 10, 8, PRINTER_FW_MEDIUM, false, false, false, 0);
  printer_select_font($handle, $font);
  printer_set_option($handle, PRINTER_MODE, "RAW");

	printer_draw_text($handle, "DutaRESTO",140,0);
	printer_draw_text($handle, date("d/m/y H:i"),200, 40);//125-200

	printer_draw_text($handle, "Waiter :", $var_magin_left, 40);
	printer_draw_text($handle, ":",45, 40);
	printer_draw_text($handle, 'Ridwan',90, 40);//80-60
	...
	printer_draw_text($handle, "Harga", 250, 70); //rubah 175 -200
	printer_draw_line($handle, $var_magin_left, 85, 400, 85);

	$row +=150;
	printer_draw_text($handle, "Terima kasih atas kunjungan anda", 20, $row);//rubah 40
