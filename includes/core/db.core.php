...
class TTable{
  ...
  function FetchPage($pagesize,$currentpage=1){
    ...
    if($this->numrows > 0){
      $str  = "<span id='' class=''>";
	  if ($currentpage>1) $str .= " <a href='?".$this->pagelinkparam."&page=1' style='color:white;'>First</a> ";
	  if ($currentpage>1) $str .= " <a href='?".$this->pagelinkparam."&page=".($currentpage-1)."' style='color:white;'>Prev</a> ";
      for($i=1;$i<=$pagenum;$i++){
        $link = $this->pagelinkparam."&page=$i";
         if ((($i >= $currentpage-3) && ($i<=$currentpage+3)) || ($i==1) || ($i == $pagenum)){   
			...
			$str .= $i == $currentpage ? " <span style='color:white;font-weight:bold;'>$i</span> " : " <a href='?$link' style='color:white;'>$i</a> ";
			$showPage = $i;
         }
      }
	  if ($currentpage < $pagenum) $str .= "<a href='?".$this->pagelinkparam."&page=".($currentpage+1)."' style='color:white;'>Next</a> ";
	  if ($currentpage < $pagenum) $str .= "<a href='?".$this->pagelinkparam."&page=".$pagenum."' style='color:white;'>Last</a> ";
      $str .= "</span>";
	  return $str;
    } else {        
      return false;
