...
$sql .= "WHERE tm.cStatus = 'O' AND hd.cStatus = 'K' AND hd.cStatKitchen = 'F' ";
$rs = $conn->Execute($sql);
$jml = $rs->fields["jml"];
...
if(($_POST['counter']!="") && ($_POST['counter']!=$jml)){
	...
	$tpl = new TSQLTemplate("kitchen.finish","../template/");
	...
	$sql .= "WHERE tm.cStatus = 'O' AND hd.cStatus = 'K' AND hd.cStatKitchen = 'F' ORDER BY hd.dTglCook ";
	$tpl->SQL = $sql;
	$data['listcust'] = $tpl->Show(false);

	$sql  = "SELECT cKdPesanan as cid FROM tr_pesananhd hd ";
	$sql .= "LEFT JOIN (SELECT cIdTamu, cStatus FROM tr_tamu) tm ON tm.cIdTamu = hd.cIdTamu ";
	$sql .= "WHERE tm.cStatus = 'O' AND hd.cStatus = 'K' AND hd.cStatKitchen = 'F' ORDER BY hd.dTglCook LIMIT 0,1 ";
	$rs = $conn->Execute($sql);
	...
	if(trim($data['idpesanan'])!=""){ 
	  $tpldt = new TSQLTemplate("kitchen.finishdt","../template/");
	  $tpldt->moduleid = "kitchen.newlistorder";
	  $sql  = "SELECT dt.cIdx, dt.cKdPesananDt, dt.cKdPesanan, dt.cKdPesanan  as IdPesanan, ";
	  $sql .= "dt.cIdTamu, dt.cKdBarang, brg.vNamaBarang, dt.vQty, dt.vQtyKitchen, COALESCE(dt.vQtyServed,0) as vQtyServed, dt.cKeterangan ";
