...
class TExcelRptSalesSummary extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	$sql  = "SELECT DATE_FORMAT(dTglInv,'%d/%m/%Y') AS dTglInv, SUM(nInvNetto) AS nInvNetto, SUM(vTotalTunai) AS vTotalTunai, SUM(vTotalDKCard) AS vTotalDKCard ";
    $sql .= "FROM tr_fakturhd WHERE MONTH(dTglInv) BETWEEN '$bulanawal' AND '$bulanakhir' AND YEAR(dTglInv) = '$tahun' ";
	$sql .= "GROUP BY DATE_FORMAT(dTglInv,'%d/%m/%Y') ORDER BY dTglInv ";  
	$rs = $conn->Execute($sql);
	...
	if($rs->RecordCount()>0){
	  ...
	  header('Content-Disposition: attachment;filename="sales_summary_'.$dtfile.'.xls"');
