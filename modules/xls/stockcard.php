...
class TExcelRptStockCard extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	$sql  = "CALL sp_kartu_stock('$tglawal','$tglakhir'); ";
 	$rs = $conn->Execute($sql);
	...
	if($rs->RecordCount()>0){
		...
		foreach($mapp_col as $k=>$v){
		  foreach($v as $sk=>$sv){
			...
			if($sk=="datetime"){
			  ...
			  //$objWorksheet->setCellValue($cell_pos,$rs->fields[$sv]);
			  $objWorksheet->setCellValueExplicit($cell_pos, $rs->fields[$sv], PHPExcel_Cell_DataType::TYPE_STRING);
			}
		  }
		}
		$row++;
	  ...
	  header('Content-Disposition: attachment;filename="stock_card_'.$dtfile.'.xls"');
